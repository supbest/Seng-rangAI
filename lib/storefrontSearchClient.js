...
function getFeaturedListingsWithIndexes() {
...
export function normalizeStorefrontSearchResponse(value) {
  ...
  if (indexes.length === 0) {
    return {
      indexes: featuredListings.map((_, index) => index),
      listings: getFeaturedListingsWithIndexes(),
      fallback: true,
      message: 'No matching storefront data found. Showing all featured listings instead.',
    };
  }
...
export async function requestStorefrontSearch(filters) {
  ...
  const { indexes, listings, fallback, message } = normalizeStorefrontSearchResponse(parsedContent);
